...
// partiendo de que ya tenemos creado un objecto PersonData previamente utilizamos el contexto
	...
	public function update_client(){
		$sql = "update ".self::$tablename." set name=\"$this->name\",email1=\"$this->email1\",address1=\"$this->address1\",lastname=\"$this->lastname\",phone1=\"$this->phone1\", company=\"$this->company\" where id=$this->id";
		Executor::doit($sql);
	}
	...
	public static function nombre_cliente($id)
	{		
		$sql = "SELECT name,lastname,numero_documento FROM person WHERE id = '".$id."' LIMIT 1";
		$query = Executor::doit($sql);
		return Model::one($query[0],new PersonData());
	}
